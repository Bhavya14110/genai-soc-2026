...
# 🛠️ FIX: System prompt belongs here, not in the memory stream
agent = create_react_agent(
    model=llm,
    tools=tools,
    checkpointer=memory,
    prompt=SYSTEM_PROMPT  # NOTE: If this throws an error, change it to `prompt=SYSTEM_PROMPT`
)
...
def run_agent_with_trace(user_input: str, session_id: str) -> tuple[str, str]:
    ...
    try:
        # 🛠️ FIX: Securely pass the user string via tuple
